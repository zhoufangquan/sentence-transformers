...
corpus_embeddings = corpus_embeddings / \
    np.linalg.norm(corpus_embeddings, axis=1)[:, None]
...
while True:
    ...
    correct_hits = util.semantic_search(
        question_embedding, corpus_embeddings, top_k=top_k_hits)[0]
    ...
    recall = len(ann_corpus_ids.intersection(
        correct_hits_ids)) / len(correct_hits_ids)
