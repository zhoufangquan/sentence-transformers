...
class WordEmbeddings(nn.Module):
    ...
    def forward(self, features):
        ...
        features.update({'token_embeddings': token_embeddings,
                        'cls_token_embeddings': cls_tokens, 'attention_mask': features['attention_mask']})
        return features
    ...
    def from_text_file(embeddings_file_path: str, update_embeddings: bool = False, item_separator: str = " ", tokenizer=WhitespaceTokenizer(), max_vocab_size: int = None):
        ...
        if not os.path.exists(embeddings_file_path):
            logger.info("{} does not exist, try to download from server".format(
            ...
            url = "https://public.ukp.informatik.tu-darmstadt.de/reimers/embeddings/" + \
                embeddings_file_path
        ...
        with gzip.open(embeddings_file_path, "rt", encoding="utf8") if embeddings_file_path.endswith('.gz') else open(embeddings_file_path, encoding="utf8") as fIn:
            iterator = tqdm(fIn, desc="Load Word Embeddings",
                            unit="Embeddings")
