...
class BoW(nn.Module):
    ...
    def __init__(self, vocab: List[str], word_weights: Dict[str, float] = {}, unknown_word_weight: float = 1, cumulative_term_frequency: bool = True):
        ...
        self.config_keys = ['vocab', 'word_weights',
                            'unknown_word_weight', 'cumulative_term_frequency']
        self.vocab = vocab
        ...
        logger.info("{} out of {} words without a weighting value. Set weight to {}".format(
    ...
    def get_sentence_features(self, tokenized_texts: List[List[int]], pad_seq_length: int = 0):
        ...
        for tokens in tokenized_texts:
            vector = np.zeros(
                self.get_sentence_embedding_dimension(), dtype=np.float32)
            for token in tokens:
