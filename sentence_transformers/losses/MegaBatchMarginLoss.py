...
class MegaBatchMarginLoss(nn.Module):
    ...
    def forward_mini_batched(self, sentence_features: Iterable[Dict[str, Tensor]], labels: Tensor):
        ...
        with torch.no_grad():
            self.model.eval()
            all_positive_emb = self.model(
                positive)['sentence_embedding'].detach()
            self.model.train()

        diagonal_matrix = torch.eye(len(all_positive_emb), len(
            all_positive_emb), device=all_positive_emb.device)
        ...
        for start_idx in range(0, len(all_positive_emb), self.mini_batch_size):
            ...
            with torch.no_grad():
                ...
                negative_scores = cos_scores - 2 * \
                    diagonal_matrix[start_idx:end_idx]
                negatives_max, negatives_ids = torch.max(
                    negative_scores, dim=1)
            ...
            positive_emb = self.model(
                {key: positive[key][start_idx:end_idx] for key in feature_names})['sentence_embedding']
            ...
            losses = F.relu(self.positive_margin - pos_cosine) + \
                F.relu(neg_cosine - self.negative_margin)
            losses = losses.mean()
    ...
    def forward_non_mini_batched(self, sentence_features: Iterable[Dict[str, Tensor]], labels: Tensor):
        ...
        losses = F.relu(self.positive_margin - positive_scores) + \
            F.relu(negatives_max - self.negative_margin)
        return losses.mean()
