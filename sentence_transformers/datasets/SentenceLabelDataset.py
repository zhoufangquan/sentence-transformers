...
class SentenceLabelDataset(IterableDataset):
    ...
    def __init__(self, examples: List[InputExample], samples_per_label: int = 2, with_replacement: bool = False):
        ...
        logger.info("SentenceLabelDataset: {} examples, from which {} examples could be used (those labels appeared at least {} times). {} different labels found.".format(
    ...
    def __iter__(self):
        ...
        while count < len(self.grouped_inputs):
            ...
            if self.with_replacement:
                ...
                selection = [i for i in np.arange(
                    left_border, right_border) if i not in already_seen[label]]
